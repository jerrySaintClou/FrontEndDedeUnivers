      ...
      try {
        ...
        for (let i = 0; i < response.data.length; i++) {
          if (response.data[i].produit.id == idProduit.idProduit) {
            ...
            setDescription(response.data[i].produit.description);
            setIdSousCategorie(response.data[i].produit.sousCategorie.id);
          }
        }
        setImageProduits(t);
        ...
        for (let i = 0; i < response.data.length; i++) {
          if (
            response.data[i].produit.sousCategorie.id ==
            product?.sousCategorie.id
          ) {
            tableauProduitSimilaire.push(response.data[i]);
          }
        }
        //setProduitSimilaire(tableauProduitSimilaire);
        setProduitSimilaire(tableauProduitSimilaire);
        // console.log(produitSimilaire);
        //   if(response.data.)
        // console.log(response.data);
    ...
    <div className={classes.productDetail}>
      <div className={classes.showProduct}>
        ...
        <div className={classes.menuProduct}>
          ...
          <button className={classes.panierStyleLiens}>
            Ajouter aux panier
          </button>
          <button className={classes.favorisStyleLiens}>
            Ajouter aux favoris
          </button>
          <button
            className={classes.boutonDescriptionStyle}
            onClick={activeDescription}
          >
            Description
          </button>
          <button
            className={classes.boutonStyleProduitSimi}
            onClick={activeSimilarProduct}
          >
            Produit similaire
          </button>
          <button className={classes.boutonStyleComm} onClick={activeComment}>
            Commentaire
          </button>
        </div>
      </div>
      ...
      >
        <div className={classes.detailMenuContent}>
          ...
          {contentMenu == "description" ? (
            <MonContexte.Provider value={{ description }}>
              <DescriptionProduct />
            </MonContexte.Provider>
          ) : contentMenu == "produit similaire" ? (
            <MonContexte.Provider value={{ produitSimilaire, idSousCategorie }}>
              <SimilarProuct />
            </MonContexte.Provider>
          ) : contentMenu == "commentaire" ? (
            <CommentProduct />
          ) : (
            <></>
          )}
